      ...
      <div className="container">
        ...
        >
          {visibleProjects.map((project) => (
            <ProjectCard key={project.id} project={project} variants={itemVariants} locale={locale} dict={dict} />
          ))}
        </motion.div>
...
function ProjectCard({ project, variants, locale, dict }: { project: Project; variants: any; locale: Locale; dict: Dictionary }) {
  // Get translated project data
  const projectTranslation = (dict.projects?.items as any)?.[project.id]
  const title = projectTranslation?.title || project.title
  const location = projectTranslation?.location || project.location
        ...
        <div className="relative aspect-[4/3] rounded-xl overflow-hidden mb-3 bg-card">
          {project.image ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-accent/10 to-accent2/10">
              <span className="text-4xl font-bold text-text/20">{title.charAt(0)}</span>
            </div>
          )}
          <button
            ...
            title={project.mapsUrl ? 'Open in Google Maps' : location}
          >
