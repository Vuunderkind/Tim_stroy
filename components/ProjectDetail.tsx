            ...
            <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-card">
              {project.image ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-accent/10 to-accent2/10">
                  <span className="text-6xl font-bold text-text/20">{title.charAt(0)}</span>
                </div>
              )}
              {project.status && (
          ...
          >
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <div className="flex items-center gap-2 text-text-muted">
                ...
                <span>{(dict.projects?.categories as any)?.[project.category] || project.category}</span>
              </div>
              <div className="flex items-center gap-2 text-text-muted">
            ...
            <div className="mt-auto pt-8 border-t border-border">
              <h3 className="text-lg font-semibold text-text mb-4">
                {(dict.projects?.details as any)?.discuss || dict.contacts?.subtitle || 'Let\'s discuss your project'}
              </h3>
              <Link
